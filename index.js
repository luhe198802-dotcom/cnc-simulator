...
app.post("/api/v1/simulate", (req, res) => {
  ...
  let pos = { x: 0, y: 0, z: 50 }, isAbs = true;
  for (const line of lines) {
    const parts = line.toUpperCase().split(/\s+/);
    let cmd = ""; const params = {};
    for (const p of parts) {
      const m = p.match(/^([A-Z])(-?[\d.]+)$/);
      if (m) { if (m[1] === "G" || m[1] === "M") cmd = m[1] + Math.floor(parseFloat(m[2])); else params[m[1]] = parseFloat(m[2]); }
    }
    if (cmd === "G90") isAbs = true;
      ...
      if (points.length === 0) points.push({ x: pos.x * scale, y: pos.y * scale, z: pos.z });
      points.push({ x: end.x * scale, y: end.y * scale, z: end.z });
      pos = end;
    }
  }
  res.json({ success: true, pathSegments: pathSegments.map(s => ({ ...s, start: { x: s.start.x * scale, y: s.start.y * scale, z: s.start.z }, end: { x: s.end.x * scale, y: s.end.y * scale, z: s.end.z } })), points, totalSegments: pathSegments.length, totalPoints: points.length, workpieceDiameter: 320 });
});
